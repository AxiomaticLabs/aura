...
mod tests {
    ...
    fn test_hybrid_document_serialization() {
        ...
        doc.data.insert("name".to_string(), DataValue::Text("Alice".to_string()));
        doc.data.insert("age".to_string(), DataValue::Integer(30));
        
        // Nested object (impossible in standard SQL, easy here)
        let mut address = HashMap::new();
        address.insert("city".to_string(), DataValue::Text("Wonderland".to_string()));
        doc.data.insert("address".to_string(), DataValue::Object(address));
        ...
        match loaded_doc.data.get("address").unwrap() {
            DataValue::Object(map) => {
                assert_eq!(map.get("city"), Some(&DataValue::Text("Wonderland".to_string())));
            },
