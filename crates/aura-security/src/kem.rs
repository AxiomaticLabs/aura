...
pub fn encapsulate(pk_bytes: &[u8]) -> Result<(Vec<u8>, Vec<u8>), CryptoError> {
    ...
    let pk = kyber1024::PublicKey::from_bytes(pk_bytes).map_err(|_| CryptoError::KemFailed)?;
...
pub fn decapsulate(
    ...
    let ciphertext =
        kyber1024::Ciphertext::from_bytes(ciphertext_bytes).map_err(|_| CryptoError::KemFailed)?;
